...
# ─── Test G5000 ───────────────────────────────────────────────────────────────
echo "Generating input for Test G5000 (5000x5000 * 5000x5000)..."
m=5000; n=5000; p=5000

# clear in and write dimensions
printf "" > in
printf "$m $n $p\n" >> in

# Generate Matrix A (25,000,000 random elements) using awk
awk -v total=$((m * n)) 'BEGIN { srand(); for(i=0; i<total; i++) printf "%d ", int(rand()*10); print "" }' >> in

# Generate Matrix B (25,000,000 random elements) using awk
awk -v total=$((n * p)) 'BEGIN { srand(); for(i=0; i<total; i++) printf "%d ", int(rand()*10); print "" }' >> in

echo "Input generated."

OUT=output/output_g5000.txt
printf "" > "$OUT"
echo "========== Test G5000: ${m}x${n} * ${n}x${p} (random matrices) ==========" >> "$OUT"
...
echo "========================================="
echo "Test G5000 complete. Results in output/output_g5000.txt"
echo "========================================="
