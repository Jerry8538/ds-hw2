...
void master_process(int m, int n, int p, int P, ifstream &inpFile) {
  ...
  // storing matrix in column major, because we need to send the columns of A in
  // contiguous manner
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
      long long x;
      ...
      int col_maj_index =
          (j * m) + i; // j * m gives us the 0th element address of the jth
                       // column then we add i to get address of the ith element
  ...
  int remainder = n % P;
  int min_pairs_to_all =
      n /
      P; // each distributed process will have atleast min_pairs_to_all pairs
  ...
  MPI_Scatter(to_send.data(), 1, MPI_INT, MPI_IN_PLACE, 0, MPI_INT, 0,
              MPI_COMM_WORLD);

  vector<int> starting_point_A(P + 1);
  vector<int> chunk_size_A(P + 1);
  vector<int> starting_point_B(P + 1);
  vector<int> chunk_size_B(P + 1);

  chunk_size_A[0] = to_send[0] * m; // master's chunk (always 0)
  chunk_size_B[0] = to_send[0] * p;
  starting_point_A[0] = 0;
  ...
  MPI_Scatterv(matrixA.data(), chunk_size_A.data(), starting_point_A.data(),
               MPI_LONG_LONG, MPI_IN_PLACE, 0, MPI_LONG_LONG, 0,
               MPI_COMM_WORLD);

  // send each process its rows of B
  MPI_Scatterv(matrixB.data(), chunk_size_B.data(), starting_point_B.data(),
               MPI_LONG_LONG, MPI_IN_PLACE, 0, MPI_LONG_LONG, 0,
               MPI_COMM_WORLD);
...
// worker process :
// in worker process we take some pairs of columns of A and rows of B and then
// find their corresponding partial matrix C and returns it to the master
void worker_process(int m, int p) {
  ...
  MPI_Scatterv(NULL, NULL, NULL, MPI_LONG_LONG, column_A.data(), m * num_pairs,
  ...
  MPI_Scatterv(NULL, NULL, NULL, MPI_LONG_LONG, row_B.data(), p * num_pairs,
               MPI_LONG_LONG, 0, MPI_COMM_WORLD);
  ...
  // here m is the rows of A, p is the columns of B and so the partial matrix C
  // will be m x p int num_pairs is the number of pairs assigned to this worker
  // process column_A is/are the columns that we need to process for this
  // particular worker process and row_B is/are the rows that we need for this
  // worker process

  vector<long long> partial_C(m * p, 0);
  int pairs_processed = 0;
  // pairs_processes also gives us the current column and row that we are
  // processing
  while (pairs_processed < num_pairs) {
    // pairs processed gives us the current column and row that we are looking
    // at
    for (int i = 0; i < m; i++) {
      // traversing the column elements of the current column
      for (int j = 0; j < p; j++) {
        int a_ind = pairs_processed * m +
                    i; // why pairs_processed * m + i ?? because pairs_processed
                       // * m gives us the 0th element of the current column of
                       // A and then we add i to get the current column element
                       // that we are looking at
        int b_ind = pairs_processed * p + j; // same logic as above

        partial_C[i * p + j] +=
            column_A[a_ind] *
            row_B[b_ind]; // i * p + j, why ?? because this gives us the index
                          // in row major form
      }
    }
    pairs_processed++;
  ...
  MPI_Reduce(partial_C.data(), NULL, m * p, MPI_LONG_LONG, MPI_SUM, 0,
             MPI_COMM_WORLD);
}
...
int main(int argc, char **argv) {
  ...
  if (rank == 0) {
    ...
    cout << "Matrix Dimensions     : A(" << m << "x" << n << ") * B(" << n
         << "x" << p << ") = C(" << m << "x" << p << ")" << endl;
