...
# ─── Test G1000 ───────────────────────────────────────────────────────────────
echo "Generating input for Test G1000 (1000x1000 * 1000x1000)..."
...
# Generate Matrix A (1,000,000 random elements) using awk
awk -v total=$((m * n)) 'BEGIN { srand(); for(i=0; i<total; i++) printf "%d ", int(rand()*10); print "" }' >> in

# Generate Matrix B (1,000,000 random elements) using awk
awk -v total=$((n * p)) 'BEGIN { srand(); for(i=0; i<total; i++) printf "%d ", int(rand()*10); print "" }' >> in

echo "Input generated."

OUT=output/output_g1000.txt
printf "" > "$OUT"
echo "========== Test G1000: ${m}x${n} * ${n}x${p} (random matrices) ==========" >> "$OUT"
...
echo "========================================="
echo "Test G1000 complete. Results in output/output_g1000.txt"
echo "========================================="
